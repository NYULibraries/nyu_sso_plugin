...
backend_port="8489"
 ...
 if File.exists?(AppConfig[:heira_path])
   ...
   heira_hash.each do |key,value|
        sso_url=value if key.includes? "sso_url"
        backend_port=value if key.includes? "backend_port"
   end
  ...
  backend_port.empty? ? AppConfig[:backend_sso_url]= "https://#{sso_url}:#{backend_port}":AppConfig[:backend_sso_url]= "https://#{sso_url}"
